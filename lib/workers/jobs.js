...
jobs.handle = function handle(p) {
  switch (p.cmd) {
    ...
    case packets.types.MINE:
      return jobs.mine(p.data, p.target, p.min, p.max, p.pow_net_type);
...
jobs.mine = function mine(data, target, min, max, pow_net_type) {
  const nonce = _mine(data, target, min, max, pow_net_type);

...
class Mempool extends EventEmitter {
  ...
  lif_kv_get(key){
    return this.lif_kv_idx.lif_kv_get(key);
  }
  ...
  lif_kv_add(tx){
    this.lif_kv_idx.add(tx);
  }
  lif_kv_rm(tx){
    const hash = tx.hash();
    this.lif_kv_idx.rm(hash);
  }
...
class MempoolOptions {
  ...
  fromOptions(options){
    ...
    if (options.lif_kv_idx!=null){
      assert(typeof options.lif_kv_idx=='boolean');

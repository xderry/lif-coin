...
class lif_kv_idx {
  ...
  get(key){
    return this.get_vals(key)?.[0]; // just return the first one in mempool
  }
  tx_get(tx_hash){
  ...
  db_op = {
    ...
    tx_kv_del: async(tx_hash)=>{
      delete this.tx_kv[revHex(tx_hash)];
    },
    kv_exists: async(key)=>{
      return !!await this.lif_kv_idx_chain.get(key);
    },

...
class AddrIndexer {
  ...
  constructor(network, options) {
    ...
    if (options?.addrshindex){
      this.index_addrsh = new BufferMap();
      this.map_addrsh = new BufferMap();
    }
  }
  ...
  reset() {
    this.index.clear();
    this.map.clear();
    if (this.index_addrsh){
      this.index_addrsh.clear();
      this.map_addrsh.clear();
    }
  }
  ...
  getKeyAddrSH(addrsh){
    let size = addrsh.length;
    const raw = Buffer.allocUnsafe(size);
    let written = 0;
    written += addrsh.copy(raw, 1);
    assert(written === size);
    return raw;
  }
  ...
  getEntries(addr, options = {}) {
    ...
    let key = addr.addrsh ? this.getKeyAddrSH(addr.addrsh) : this.getKey(addr);
    if (!key)
      return [];
    let items = addr.addrsh ? this.index_addrsh.get(key) : this.index.get(key);
    if (!items)
      return [];
    let values = [];
  ...
  insert_addrsh(entry, view){
    const tx = entry.tx;
    const hash = tx.hash();
    const addrs = tx.getAddressesSH(view);
    if (!addrs.length)
      return;
    for (const addrsh of addrs){
      const key = this.getKeyAddrSH(addrsh);
      if (!key)
        continue;
      let items = this.index_addrsh.get(key);
      if (!items){
        items = new BufferMap();
        this.index_addrsh.set(key, items);
      }
      assert(!items.has(hash));
      items.set(hash, entry);
    }
    this.map_addrsh.set(hash, addrs);
  }

  remove(hash){
    if (this.addrshindex)
      this.remove_addrsh(hash);
  ...
  remove_addrsh(hash){
    const addrs = this.map_addrsh.get(hash);
    if (!addrs)
      return;
    for (const addrsh of addrs){
      const key = this.getKeyAddrSH(addrsh);
      if (!key)
        continue;
      const items = this.index_addrsh.get(key);
      assert(items);
      assert(items.has(hash));
      items.delete(hash);
      if (!items.size)
        this.index_addrsh.delete(key);
    }
    this.map_addrsh.delete(hash);
  }
}

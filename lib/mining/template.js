...
class BlockTemplate {
  ...
  fromOptions(options) {
    ...
    if (options.address != null)
      this._setAddress(options.address);
    if (options.network != null)
      this.network = options.address;
  ...
  createCoinbase(segwit_hash) {
    ...
    assert(this.address, 'mine: no address defined');
    // Reward outputs.
    let value_sum = this.address.reduce((sum, a)=>sum+(a.value||0), 0);
    ...
    for (const a of this.address){
      ...
      output.value = a.rest ? value_rest : a.value;
  ...
  _setAddress(address){
    if (typeof address=='string')
      this.address = [{address: new Address(address), rest: true}];
      ...
      for (const a of address){
        ...
        if (a.value!=null && a.rest)
          throw Error('rest cannot have value set');
        if (a.rest){
          if (rest)
            throw Error('only one coinbase address can be rest');
          rest++;
        }
        this.addresss.push({...a, address: addr});
      }
      if (rest!=1)

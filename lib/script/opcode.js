...
class Opcode {
  ...
  toFormat(){
    ...
    if (this.data){
      ...
      // Direct push
      if (symbol && symbol.startsWith('OP_PUSHBYTES')){
  ...
  toASM(decode){
    ...
    if (this.data){
      data += ' ';
      if (this.value>=opcodes.OP_PUSHBYTES1 && this.value<=opcodes.OP_PUSHBYTES75){
        data += this.data.toString('hex');
      }
      else
        data += common.toASM(this.data, decode);
    }
    return (common.opcodesByVal[this.value] || 'OP_UNKNOWN_'+this.value)+data;
  }

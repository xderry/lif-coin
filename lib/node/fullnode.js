...
class FullNode extends Node {
  ...
  constructor(options){
    ...
    this.mempool = new Mempool({
      ...
      lif_kv_idx: this.config.bool('lif_kv_idx'),
    });
    // Indexers
  ...
  async lif_kv_get(key, options = {}){
    let confirmed = await this.lif_kv_idx.lif_kv_get(key);
    let unconfirmed = await this.mempool.lif_kv_get(key);
    return confirmed || unconfirmed;
  }
  async lif_kv_get_vals(key, options = {}){
    let unconfirmed = await this.mempool.lif_kv_get_vals(key);
    let confirmed = await this.lif_kv_idx.lif_kv_get_vals(key);
    return {confirmed, unconfirmed};
  }

...
class FullNode extends Node {
  ...
  async lif_kv_get(key){
    let confirmed = await this.lif_kv_idx.get(key);
    if (confirmed!==undefined)
      return confirmed;
    let unconfirmed = await this.mempool.lif_kv_idx.get(key);
    return unconfirmed;
  }
  async lif_kv_get_vals(key){
    let unconfirmed = await this.mempool.lif_kv_idx.get_vals(key);
    let confirmed = await this.lif_kv_idx.get_vals(key);
    return {confirmed, unconfirmed};
  }
  async lif_kv_tx_get(tx_hash){
    if (this.mempool.getTX(tx_hash))
      return await this.mempool.lif_kv_idx.tx_get(tx_hash);
    return await this.node.lif_kv_idx.tx_get(tx_hash);
  }

...
class HTTP extends Server {
  ...
  init_electrum(){
    ...
    ws_rpc.method('blockchain.scripthash.get_balance', async(sh_a)=>{
    ...
    ws_rpc.method('blockchain.scripthash.get_history', async(sh_a)=>{
      let his = [];
      for (let sh of sh_a){
        let addrsh = electrum_addrsh(sh);
        let metas = await this.node.getMetaByAddrSH({addrsh});
        for (const meta of metas)
          his.push({height: meta.height, tx_hash: meta.tx.rhash()});
      }
      his.sort(height_cmp);
      return his;
    });

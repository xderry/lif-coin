...
class HTTP extends Server {
  ...
  init_electrum(){
    ...
    ws_rpc.method('blockchain.mine.get_template', async([addr])=>{
      let _addr = new Address(addr);
      let offer = await this.mine_attempt_get();
    ...
    ws_rpc.method('blockchain.mine.submit_claim', async([header])=>{
      const _header = Buffer.from(header, 'hex');
      let res = await this.mine_attempt_claim(_header);
      if (!res)
  ...
  async mine_attempt_get({addr, addr_self, nonce1, nonce2}){
      ...
      mo.time<=time-this.mine_assmble_refresh)))
    {
      ...
      mo.attempt_wait = this.miner.createBlock(null, this.node.address);
    }
    if (!mo.attempt)
    ...
    let offer = {
      ...
      saddr,
      saddr_self,
      reward_share: 0.9, // 90% reward to miner, 10% to this node (self)
      coinbase_raw: attempt.getRawCoinbase(nonce1, nonce2),
      header: attempt.getHeader(
        attempt.getRoot(offer.nonce1, offer.nonce2), attempt.time, 0),
    };
    mo.key = this.mine_header_key(offer.header),
  ...
  async mine_attempt_claim(header){
    const hash = this.network.pow_hash256.digest(header);
    ...
    if (!found)
      return console.log('not a winning target claim', _header);
    const key = this.mine_header_key(header);
    const mo = this.mo;
    if (!mo)
      return console.log('winning target without offer', _header);
    const offer = mo.offer[key];
    if (!offer)
      return console.log('winning target wihtout offer found', _header);
    // add block offer.attempt;
    const block = offer.attempt.toBlock();
    let entry = await this.chain.add(block);
    return entry;
  }

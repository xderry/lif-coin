...
class HTTP extends Server {
  ...
  init_electrum(){
    ...
    ws_rpc.method('blockchain.mine.get_template', async([addr])=>{
      ...
      const attempt = await this.miner.createBlock(null, _addr);
      const block = attempt.toBlock();
      const nonce = {time: attempt.time, n0: 0, n1: 9, n2: 0};
      const header = attempt.getHeader(attempt.getRoot(nonce.n1, nonce.n2),
        attempt.time, nonce.n0);
      console.log(revHex(header));
      let ret = {header: header.toString('hex')};
      if (1){

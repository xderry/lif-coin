...
class HTTP extends Server {
  ...
  initRouter(){
    ...
    let blockstream_address = async(req, res)=>{
      ...
      if (this.node.lif_kv_idx){
        let kvs = await this.lif_kv_metas_sum(metas, a.addrsh);
        sum.lif_kv = kvs;
      }
      res.json(200, sum);
    };
  ...
  init_electrum(){
    ...
    ws_rpc.method('blockchain.scripthash.get_balance', async([sh])=>{
      ...
      if (this.node.lif_kv_idx)
        bal.lif_kv = await this.lif_kv_metas_sum(metas, addrsh);
      return bal;
    });
  ...
  async lif_kv_metas_sum(metas, addrsh){
    ...
    for (const meta of metas){
      ...
      let tx_kv = await this.node.lif_kv_idx.tx_get(tx.hash());
      if (!tx_kv)
        continue;
      for (let i=0; i<tx.outputs.length; i++){
        let out = tx.outputs[i];

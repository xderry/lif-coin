...
class HTTP extends Server {
  ...
  initRouter(){
    ...
    let blockstream_address = async(req, res)=>{
      ...
      if (this.node.lif_kv_idx){
        let kvs = await lif_kv_metas_sum(metas, a.addrsh);
        sum.lif_kv = kvs;
      }
      res.json(200, sum);
    };
    ...
    this.get('/lif_kv', async(req, res)=>{
      ...
      let val = await this.node.lif_kv_get(key);
      res.json(200, val);
    });
  ...
  init_electrum(){
    ...
    ws_rpc.method('blockchain.scripthash.get_balance', async([sh])=>{
      ...
      if (this.node.lif_kv_idx)
        bal.lif_kv = await lif_kv_metas_sum(metas, addrsh);
      return bal;
    });
    ...
    ws_rpc.method('blockchain.transaction.get', async([tx_hash, verbose])=>{
      ...
      if (!meta)
        throw new Error(`TX not found: ${tx_hash}`);
      let tx = meta.tx;
      if (!verbose)
  ...
  async blockstream_tx(tx){
    ...
    let lif_kv;
    if (this.node.lif_kv_idx){
      lif_kv = await this.node.lif_kv_tx_get(tx.hash());
      console.log('lif_kv', lif_kv);
    }
...
async function lif_kv_metas_sum(metas, addrsh){

...
class TX {
  ...
  getInputAddressesSH(view){
    ...
    for (const input of this.inputs){
      ...
      const addrsh = coin.sha256();
      if (!addrs.has(addrsh)) // XXX FIXME: addrs.has()
        addrs.push(addr);
    }
    return addrs;
  }
  ...
  getOutputAddressesSH(){
    ...
    for (const output of this.outputs){
      ...
      if (!addrs.has(addrsh)) // XXX FIXME: addr.has()
        addrs.push(addrsh);
    }
    return addrs;
  }
  ...
  getAddresses(view){
    ...
    const outputs = this.getOutputAddressesSH();
    for (const addr of outputs){
      if (!addrs.has(addrsh)) // XXX FIXME: addrs.has()
        addrs.push(addr);
    }
    return addrs;

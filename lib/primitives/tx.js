...
class TX {
  ...
  getInputAddresses(view){
    ...
    return addr_unique(addrs);
  }
  getInputAddressesSH(view){
    const addrs = [];
    if (this.isCoinbase() || !view)
      return [];
    for (const input of this.inputs){
      const coin = view.getOutputFor(input);
      const addrsh = coin.script.sha256();
      addrs.push(addrsh);
    }
    return addrsh_unique(addrs);
  }
  ...
  getOutputAddressesSH(){
    const addrs = [];
    for (const output of this.outputs){
      if (output.script.isUnspendable())
        continue;
      const addrsh = output.script.sha256();
      addrs.push(addrsh);
    }
    return addrsh_unique(addrs);
  }
  ...
  getAddresses(view){
    ...
    addrs.push(...this.getOutputAddresses());
    return addr_unique(addrs);
  }
  getAddressesSH(view){
    const addrs = this.getInputAddressesSH(view);
    addrs.push(...this.getOutputAddressesSH());
    return addrsh_unique(addrs);
  }
  ...
  getInputHashes(view, enc){
    const hashes = this.getInputAddresses(view).map(a=>a.getHash());
    if (enc!='hex')
      return hashes;
    return hashes.map(h=>h.toString('hex'));
  }
   ...
   */
  getOutputHashes(enc){
    const hashes = this.getOutputAddresses().map(a=>a.getHash());
    if (enc!='hex')
      return hashes;
    return hashes.map(h=>h.toString('hex'));
  }
   ...
   */
  getHashes(view, enc){
    const hashes = this.getAddresses(view).map(a=>a.getHash());
    if (enc!='hex')
      return hashes;
    return hashes.map(h=>h.toString('hex'));
  }
...
// unique addr/addrsh hashes
function addr_unique(_addrs){

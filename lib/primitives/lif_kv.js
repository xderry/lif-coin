...
// XXX mvoe to utils/util.js
let OF = o=>o ? Object.entries(o) : [];
function obj_find(o, fn){
  return Object.entries(o).find(([k, v])=>fn(k, v));
}

function tx_merge(tx){
  let vout = {};
  let [pos_def] = obj_find(tx.vout, (i, kv)=>kv.spend) || [0];
  let kvs = [];
  let vin_keys = {};
  // prepare all keys needed to be mapped to vouts
  for (let [vin, _kv] of OF(tx.vin)){
    for (let kv of _kv){
      kvs.push({pos: 0, vin: +vin, kv});
      vin_keys[kv.key] = true;
    }
  }
  let nout = 0;
  for (let [pos, kv] of OF(tx.vout)){
    pos = +pos;
    nout = Math.max(nout, pos+1);
    if (!kv || kv.key==null)
      continue;
  ...
  for (let j=0; j<kvs.length; j++){
    ...
    for (; pos<nout && !tx.vout[pos].spend; pos++);
    if (pos==nout)
      pos = pos_def;
...
async function idx_tx_add(tx, db_op){
  ...
  for (let i=0; i<tx.inputs.length; i++){
    ...
    let p = input.prevout;
    let p_kv = await db_op.tx_kv_get(p.hash);
    if (!p_kv)
      continue;
    if (!p_kv.vout[p.index])
      continue;
    is_kv = true;
    vin[i] = p_kv.vout[p.index];
  }
  ...
  for (let i=0; i<tx.outputs.length; i++){
    ...
    vo.exists = (await db_op.kv_get(kv.key))!==undefined;
    vo.key = kv.key;
    vo.val = kv.val;
    is_kv = true;
  }
  if (!is_kv)
    return;
  // update DB
  let _vout = tx_merge({vin, vout});
  await db_op.tx_kv_put(tx.hash(), _vout);
  for (let [pos, _kv] of OF(_vout)){
    for (let kv of _kv)
      await db_op.kv_put(tx.hash(), kv.key, kv.val);
  }
}
...
async function idx_tx_rm(tx, db_op){
  ...
  for (let [vout, kvs] of OF(vals)){
    for (let p of kvs){
      ...
      let p_val;
      if (!(p_val = p_kv.vout[p.index])){
        console.warn(`failed missing prev kv ${key} ${vin}`);
        continue;
      }
      if (p_val==null)
        await db_op.kv_del(tx.hash(), key);
      else
        await db_op.kv_put(tx.hash(), key, p_val);
    }
  }
